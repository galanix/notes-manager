...
function renderFolderSelect<T>(arr: T[], counter: number) {
	for(let i = 0; i < arr.length; i++) {
		let item: any = arr[i];
...
function renderTagSelect<T>(arr: T[], counter: number) {
	for(let i = 0; i < arr.length; i++) {
		let item: any = arr[i];
		let name: string = item.name;
		let id: number = item.id;
...
function parseFolders(folders: any) { 
	let ul: any = $("<ul>");
...
function parseFolder(folder: any) { 
	let li: any = $("<li>");
...
function renderFoldersDisplay<T>(folders: T[]) { 
...
function renderTagsDisplay<T>(tags: T[]) { 
	for (let key in tags) {
...
function renderNoteTagsDisplay<T>(tags: T[]) {
	for (let key in tags) {
		let item: any = tags[key];
		let name: string = item.name;
...
// Math max to array
function getMaxOfArray<T>(numArray: T[]): T {
	return Math.max.apply(null, numArray);
}

// Find last(date) note and return it as object from data
function findLatestNote<T>(): any {
	let dataArr: T[] = [];
	let maxNote: any;
	...
	let max: any = getMaxOfArray(dataArr);
	for(let i = 0; i < data.notes.length; i++) {
		let item: any = data.notes[i];
		let parseDate: any = Date.parse(item.date);
		if ( parseDate == max ) {
...
function renderTags() {
	...
	function renderNotes<T>(folders: T[]) {
